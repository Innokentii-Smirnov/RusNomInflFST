        ...
        with open(filename, 'r', encoding='utf-8') as fin:
            for line in fin:
                ...
                if '=' in line:
                    ...
                    try:
                        grammemes, implied_categories = re.split(' -/-> ', line)
                    except ValueError:
                        raise ValueError(line)
                    for grammeme in split(r' \| ', grammemes):
                        for implied_category in split(r' \| ', implied_categories):
                            cats = [cat for cat, vals in properties.items() if grammeme in vals]
                            assert  len(cats) == 1, 'Ambiguiuty: ' + line 
                            neg_implic.append(((cats[0], grammeme), implied_category))
                elif '->' in line:
                    try:
                        grammemes, implied_categories = re.split(' -> ', line)
                    except ValueError:
                    ...
                    for grammeme in split(r' \| ', grammemes):
                        for implied_category in split(r' \| ', implied_categories):
                            cats = [cat for cat, vals in properties.items() if grammeme in vals]
                            assert  len(cats) == 1, 'Ambiguiuty: ' + line 
                            pos_implic.append(((cats[0], grammeme), implied_category))
                else:
                    raise ValueError(line)
